...
class MyWidget(QWidget):
    ...
    def __init__(self, autocomplete_words, index_data):
        ...
        # Start the deferred indexing thread for media files
        from smartlex.gui.threads import DeferredIndexingThread
        self.deferred_thread = DeferredIndexingThread()
        self.deferred_thread.new_data_signal.connect(self.on_deferred_data_indexed)
        self.deferred_thread.start()
        
        self.initUI()

    def on_deferred_data_indexed(self, file_path, new_keywords):
        """Called when the background deep indexer finishes a media file."""
        if file_path not in self.index_data:
            self.index_data[file_path] = []
            
        # Combine existing keywords (like filename metadata) with new deep keywords
        combined = set(self.index_data[file_path] + new_keywords)
        self.index_data[file_path] = list(combined)
        
        # Save to output.json silently
        from smartlex.core.config import load_config
        from smartlex.core.index_manager import save_index
        cfg = load_config()
        save_index(self.index_data, cfg["OUTPUT_FILE"])
        
        self.status_label.setText(f"Background ML indexed: {Path(file_path).name}")
        self.status_label.setStyleSheet("padding: 10px; background-color: #f0fdf4; border-radius: 8px; border: 1px solid #bbf7d0; color: #166534; font-weight: bold;")
        logger.info(f"Deep index updated for {file_path}")

    def closeEvent(self, event):
        """Cleanup threads when window closes."""
